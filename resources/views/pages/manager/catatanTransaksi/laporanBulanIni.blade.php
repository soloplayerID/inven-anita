...
<head>
    ...
    <title>Laporan Pendapatan - {{ \Carbon\Carbon::now()->setTimezone('Asia/Jakarta')->isoFormat(' MMMM YYYY ') }}
    </title>
</head>
...
<body>
    ...
    <?php
          ...
          <center><img src="{{ public_path('/fotoSetting/'.$setting->logo) }}" style="width: 50%; max-width: 200px" /></center>
			<center><h5>{{$setting->name_application }}</h5></center><br><pre >Jl. Raya Hankam Ujung Aspal RT 004/006 No. 428 Kel. Jatimurni<br> Kec. Pondok Melati, Kota Bekasi, 17431</pre>
			Date: {{ \Carbon\Carbon::now()->setTimezone('Asia/Jakarta')->isoFormat('DD MMMM YYYY ') }} <center><h4 style="margin-bottom: 10px;">Laporan Pendapatan</h4></center>
